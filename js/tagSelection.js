//let select all required tags or element
...
function prevMusic(){
    ...
    musicIndex--;
    //musicindex is less than 1 musicIndex will be 1 so the first song will play
...
playPauseBtn.addEventListener("click", ()=>{
    ...
    //if isMusicPaused is true then call pauseMusic else player
...
mainAudio.addEventListener("timeupdate", (e)=>{
    ...
    progressBar.style.width = `${progressWidth}%`;
        ...
        if(totalSec < 10){ //adding 0 if sec less than 10
            totalSec = `0${totalSec}`;
        }
    ...
    if(currentSec < 10){ //adding 0 if sec less than 10
...
//let update playing song current time according to the progress bar wwidth
...
repeatBtn.addEventListener("click", ()=>{
    ...
    //let's do different chanh=ge on different icon click using switch
    switch(getText){
...
mainAudio.addEventListener("ended", ()=>{
    //we'll do according to icon means if user has icon to loop song then we'll repeat
    //the current song and will do further accordingly

    let getText = repeatBtn.innerText; //getting innertext of icon 
    //let's do different chanh=ge on different icon click using switch
    switch(getText){
        case "repeat": //if this icon is repeat then simply we call the nextMusic function so the next song
        nextMusic();
        break;
        case "repeat_one": //if icon is repeat_one then we'll change the current playing song current the song
...
for(let i = 0; i < allMusic.length; i++) {
    ...
    let liTag = `<li li-index="${i + 1}">
                    ...
                    <span id="${allMusic[i].src}" class="audio-duration">7:03</span>
                </li>`;
...
function playingNow(){
    for (let j = 0; j < allLiTags.length; j++) {
        ...
        //let remove playing class from all other li expect the last one which
        if(allLiTags[j].classList.contains("playing")){
            allLiTags[j].classList.remove("playing");
            //let's get that audio duration value and pass to audio-duration innertext
            let adDuration = audioTag.getAttribute("t-duration");
            audioTag.innerText = adDuration;//passing t0 duration value to audio duration duration
